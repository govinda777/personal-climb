...
app.post("/actions/onboarding", async (c) => {
  ...
  try {
    const profile = await gamificationService.addXP(
      user.id,
      reward.XP,
      reward.REASON,
...
app.get("/verify-xp/:address", async (c) => {
  ...
  if (
    ...
    return c.json(
      {
        error:
          "Server misconfiguration: Missing SIGNER_PRIVATE_KEY in production",
      },
      500,
    );
  }
  ...
  if (!privateKeyRaw.startsWith("0x")) {
    return c.json(
      { error: "Server misconfiguration: Invalid private key format" },
      500,
    );
  }
