...
test.describe('Gamification Web3 E2E', () => {
  ...
  test('Should trigger onboarding and retrieve EIP-712 XP proof (Mocked API)', async ({ page }) => {
    ...
    await page.route('**/api/actions/onboarding', async (route) => {
      ...
      const hasOnboarded = route.request().headers()['x-has-onboarded'] === 'true';
      if (hasOnboarded) {
          await route.fulfill({
            status: 429,
            contentType: 'application/json',
            body: JSON.stringify({ error: 'Rate limit exceeded for action: Onboarding Complete. Cooldown is 8760 hours.' }),
          });
      } else {
          await route.fulfill({
            status: 200,
