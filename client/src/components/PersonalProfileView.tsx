...
import { User, Briefcase, Mountain, CheckCircle2, Shield, Zap, Target } from "lucide-react";
...
export default function PersonalProfileView({ slug }: { slug: string }) {
  ...
  const handleJoinProtocol = async (packageId: string) => {
    ...
    try {
      ...
      const sessionData = await createCheckoutSession(packageId, personalData.id, token || '');
      if (sessionData.url) {
  ...
  if (loading || !personalData) {
     return <div className="min-h-screen bg-black flex items-center justify-center text-white"><span className="animate-pulse">Loading Protocol...</span></div>;
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black text-white selection:bg-white selection:text-black font-sans">
      ...
      <nav className="fixed w-full z-50 flex items-center justify-between p-6 bg-black/80 backdrop-blur-md border-b border-white/5">
        ...
        <Button variant="outline" onClick={() => !authenticated && login()} className="border-white/10 text-[10px] font-black tracking-widest px-6 h-10 hover:bg-white hover:text-black">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-20">
        ...
        >
          ...
          <h1 className="text-7xl md:text-[12rem] font-black tracking-tighter leading-[0.8] italic uppercase">
            {(personalData.heroTitle || mockConfig.heroTitle).split(' ').map((word: string, i: number) => (
          ...
          <div className="flex flex-wrap justify-center gap-6 pt-12">
            <Button onClick={() => document.getElementById('planos')?.scrollIntoView({ behavior: 'smooth' })} size="lg" className="bg-white text-black hover:bg-zinc-200 h-16 px-12 rounded-none font-black italic text-xl uppercase tracking-tighter group">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-24 flex flex-col md:flex-row justify-between items-end gap-8">
            ...
            <p className="text-zinc-500 max-w-md font-bold italic uppercase text-right">
              {personalData.philosophy || personalData.trainingPhilosophy || mockConfig.philosophy}
            </p>
          </div>
      ...
      <section id="planos" className="py-32 px-6">
        <div className="max-w-5xl mx-auto space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
            {(personalData.packages || mockConfig.packages).map((pkg: any, i: number) => (
              <div key={i} className="relative group overflow-hidden border border-white/5 bg-zinc-900/30 p-12">
                ...
                <div className="text-6xl font-black tracking-tighter mb-12">R$ {pkg.price}</div>
                ...
                <Button onClick={() => handleJoinProtocol(pkg.id || 'mock')} className="w-full h-16 rounded-none bg-white text-black font-black italic uppercase tracking-tighter text-lg hover:bg-zinc-200">
      ...
      <footer className="p-12 md:p-24 border-t border-white/5 bg-zinc-950">
        ...
        <div className="mt-24 pt-12 border-t border-white/5 text-center text-[10px] font-black text-zinc-800 uppercase tracking-[0.5em]">
          © 2024 {personalData.brandName || mockConfig.brandName} {"//"} Powered by Personal Climb OS
        </div>
      </footer>
    </div>
  );
