...
import { fetchPersonalBySlug, createCheckoutSession } from "@/lib/api";
import { personalConfig as mockConfig } from "@/lib/mockConfig";

export default function PersonalProfileView({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const data = await fetchPersonalBySlug(slug);
        setPersonalData(data);
      } catch (err) {
        console.error("Failed to load real personal data, using mock.", err);
        setPersonalData({ ...mockConfig, isMock: true });
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleJoinProtocol = async (packageId: string) => {
    if (!authenticated) {
      login();
      return;
    }
    ...
    <div className="flex flex-col min-h-screen bg-black text-white selection:bg-white selection:text-black font-sans">
      ...
      <nav className="fixed w-full z-50 flex items-center justify-between p-6 bg-black/80 backdrop-blur-md border-b border-white/5">
        <div className="flex items-center gap-2 font-black text-2xl tracking-tighter italic">
          ...
          <span>{personalData.brandName?.toUpperCase() || "PROTOCOL"}</span>
        </div>
        ...
        >
          {authenticated ? "STUDENT PORTAL" : "LOGIN"}
        </Button>
      </nav>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-20">
        ...
        >
          ...
          <h1 className="text-7xl md:text-[12rem] font-black tracking-tighter leading-[0.8] italic uppercase">
            {(personalData.heroTitle || mockConfig.heroTitle)
              .split(" ")
              .map((word: string, i: number) => (
                <span key={i} className={i % 2 === 1 ? "text-zinc-800" : ""}>
                  {word}{" "}
                </span>
              ))}
          </h1>
          <p className="text-lg md:text-xl text-zinc-500 max-w-2xl mx-auto font-bold italic uppercase tracking-tight">
            {personalData.heroSubtitle || mockConfig.heroSubtitle}
          </p>
          <div className="flex flex-wrap justify-center gap-6 pt-12">
            ...
            <Link href="/business">
              <Button
                size="lg"
                variant="outline"
                className="border-white/10 h-16 px-12 rounded-none font-black italic text-xl uppercase tracking-tighter group"
              >
      ...
      <section
        id="agendamento"
        className="py-32 px-6 max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-20 items-center"
      >
        <div className="space-y-8">
          <h2 className="text-5xl md:text-7xl font-black italic tracking-tighter uppercase leading-none">
            Sincronize <br />
            <span className="text-zinc-700">Seu Treino.</span>
          </h2>
          <p className="text-zinc-500 text-lg font-bold italic uppercase">
            Sistema de check-in em tempo real. Combine seus horários com o
            personal e garanta sua vaga na sessão.
          </p>
          <ul className="space-y-4 pt-4">
            {[
              "Confirmação Instantânea",
              "Limite de Alunos por Sessão",
              "Integração com Agenda Google",
            ].map((item, i) => (
              <li
                key={i}
                className="flex items-center gap-3 text-zinc-300 font-bold italic uppercase text-sm"
              >
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-24 flex flex-col md:flex-row justify-between items-end gap-8">
            <h2 className="text-5xl md:text-8xl font-black italic tracking-tighter uppercase leading-none">
              Nosso <br />
              <span className="text-zinc-800">Protocolo.</span>
            </h2>
            <p className="text-zinc-500 max-w-md font-bold italic uppercase text-right">
              {personalData.philosophy ||
                personalData.trainingPhilosophy ||
                mockConfig.philosophy}
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: "Coleta de dados antropométricos e histórico de lesões via interface otimizada.",
                icon: Shield,
              },
              {
                title: "Métricas de Carga",
                desc: "Cálculo automatizado de volume e intensidade baseado no seu RPE.",
                icon: Zap,
              },
              {
                title: "Evolução RPG",
                desc: "Transforme sua força em atributos visuais e acompanhe seu crescimento.",
                icon: Target,
              },
            ].map((item, i) => (
              <div
                key={i}
                className="bg-black p-12 space-y-6 hover:bg-zinc-900 transition-colors group"
              >
                <item.icon className="w-10 h-10 text-zinc-700 group-hover:text-white transition-colors" />
                <h3 className="text-2xl font-black italic uppercase tracking-tighter">
                  {item.title}
                </h3>
                <p className="text-zinc-500 text-sm font-bold italic uppercase leading-relaxed">
                  {item.desc}
                </p>
              </div>
            ))}
      ...
      <section id="planos" className="py-32 px-6">
        <div className="max-w-5xl mx-auto space-y-20">
          <div className="text-center">
            <h2 className="text-6xl font-black italic tracking-tighter uppercase mb-4">
              Investimento
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {(personalData.packages || mockConfig.packages).map(
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              (pkg: any, i: number) => (
                <div
                  key={i}
                  className="relative group overflow-hidden border border-white/5 bg-zinc-900/30 p-12"
                >
                  ...
                  <div className="text-6xl font-black tracking-tighter mb-12">
                    R$ {pkg.price}
                  </div>
                  <div className="space-y-4 mb-12">
                    {(pkg.features || []).map((f: string, j: number) => (
                      <div
                        key={j}
                        className="flex items-center gap-3 text-zinc-400 font-bold italic uppercase text-[10px] tracking-widest"
                      >
                  ...
                  <Button
                    onClick={() => handleJoinProtocol(pkg.id || "mock")}
                    className="w-full h-16 rounded-none bg-white text-black font-black italic uppercase tracking-tighter text-lg hover:bg-zinc-200"
                  >
                    {authenticated ? "Join Protocol" : "Sign in to Join"}
                  </Button>
                </div>
              ),
            )}
      ...
      <footer className="p-12 md:p-24 border-t border-white/5 bg-zinc-950">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-16">
          <div className="space-y-6">
            ...
            <p className="text-zinc-600 max-w-xs font-bold italic uppercase text-xs tracking-widest">
              The ultimate high-performance operating system for climbing
              coaches and athletes.
            </p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-20">
            <div className="space-y-4">
              <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.4em]">
                Social
              </h4>
        ...
        <div className="mt-24 pt-12 border-t border-white/5 text-center text-[10px] font-black text-zinc-800 uppercase tracking-[0.5em]">
          © 2024 {personalData.brandName || mockConfig.brandName} {"//"} Powered
          by Personal Climb OS
        </div>
      </footer>
    </div>
  );
