...
envs.forEach(({ example, target }) => {
  if (!fs.existsSync(target)) {
    if (fs.existsSync(example)) {
      ...
      console.log(`✅ Created ${path.relative(path.join(__dirname, '..'), target)} from example.`);
    } else {
      console.warn(`⚠️ Warning: Example file not found at ${example}`);
    }
  } else {
    console.log(`ℹ️  ${path.relative(path.join(__dirname, '..'), target)} already exists, skipping.`);
  }
});
